...
## 📌 02. 개념 모델링 : 개략적인 틀을 구성해 놓는 단계, 즉 분석 단계  
...
> ### 👉 논리 모델링을 바탕으로 스키마 구조 구현하는 단계
- 엔티티 -> 테이블, 속성 -> 컬럼, 관계 및 무결성 설계, 반정규화, 중복 컬럼 추가 


<br>

### 2. 테이블 설계 : '로우'와 '칼럼'으로 구성되는 가장 기본적인 오브젝트
...
- 배타적 관계를 없도록 구성 -> 테이블을 통합 / 기본키(PK) 구조를 통일 
...
- 데이터 타입 및 길이 지정
  - CHAR(n) : 고정길이 문자 데이터타입, n 바이트 길이, 최대 2,000까지 -> 'n길이 만큼 공백을 채우므로 저장공간 낭비할 수 있음'. 즉, 웬만해선 사용 x
  - VARCHAR2(n) : 가변길이 문자 데이터 타입, CHAR와 차이점은 공백이 안채워짐 
  - NCHAR(N) : 유니코드 형식의 데이터를 포함하는 고정길이/가변길이 문자 데이터 타입
  - NUMBER : 38자리 정밀도 숫자를 저장할 수 있고, 최대 22 바잍 까지 길이로 저장함
  - DATE : 고정길이 7 바이트의 날짜와 시간을 저장 

<br>

- 현업 이슈
  - (1) CHAR vs VARCHAR2 -> VARCHAR2 사용
    - CHAR는 공백이 채워지는 게 문제, 1자리는 괜찮음. 예를들어 성별 구별하는 코드
    - 하지만, 그 외의 타입은 VARCHAR2 사용하는게 좋음

  - (2) 일자(YYYYMMDD) 컬럼은 VARCHAR2 vs DATE -> 상황에 따라 다름
...
- 기본키(PK) 지정
  - 코드 성격의 컬럼보다는 고객번호처럼 값의 종류가 많은 컬럼이 먼저 오도록 구성하는 것이 좋음
    - 기본키의 선두 컬럼이 아닌 컬럼으로 구성된 인덱스를 추가로 만들지 않으므로 더 효율적임 
  - 주소록 : 고객번호 + 주소번호 -> '고객번호' 선두컬럼 설정
  - 여러개 뭉텅이를 들고 올라가면 index를 통해 정렬로 쭉 가져옴, 단건만 가지고 오면 서브 쿼리로 가져옴 
...
- 성능을 고려한 데이터 구조, 크게 3가지 방식이 있음
  - (1) 집계/요약 테이블 추가 : 테이블에서 대량의 데이터를 읽음, 이자나 부서 단위로 집계, 업무 데이터를 처리하면서 일부 데이터를 집계하는 경우 -> 집계 테이블 추가
